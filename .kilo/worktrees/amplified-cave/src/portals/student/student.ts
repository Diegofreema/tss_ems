...
/**
 * The two things the student's own pages say about them beside their name: what
 * class they are in, and whether the office is still waiting on a fee.
 */

/**
 * The arm is the more particular of the two, and the one a student would name if
...
/**
 * Where the student stands on fees, counted from their own bills.
 *
 * `GET /students/me/dashboard` is not the place to count this: its fee
 * counters are not scoped to the caller. Student 4's says four invoices with one
 * unpaid while their ledger — and the office's, which is the same three rows —
 * is settled in full; the bill it counts is another student's. A tag telling a
 * student they owe money they do not is the one wrong answer that matters here.
 */
