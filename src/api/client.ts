...
/**
 * Always same-origin, at `/backend/api`.
 *
 * Laravel is installed at `/backend` and its own routes carry the `/api`
 * prefix, so the two stack — `/backend/users/me` is a 404, and a 404 there
 * carries no `Access-Control-Allow-Origin`, so the browser reports the wrong
 * path as a CORS failure and hides what actually happened.
 *
 * Same-origin rather than the school's host written out: an absolute base
 * skips Vite's proxy and the rewrite in `vercel.json` entirely, so every dev
 * request becomes a cross-origin one to a host this app does not control —
 * and if that host's certificate has expired, the handshake fails before any
 * CORS header can arrive and the browser calls that a CORS error too.
 *
 ...
 * `new URL(path, base)`, which rejects a relative base. Off `globalThis`
 * rather than `window`, with a stand-in origin where there is none: this
 * module is imported by tests as well as by the browser, and it used to throw
 * on the way in wherever `window` was not defined.
 */
