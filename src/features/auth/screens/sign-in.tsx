...
const DISABLED_FAILURE: Failure = {
  ...
  const onSubmit = async (values: SignInValues) => {
    ...
    try {
      ...
      if (!account) {
        setFailure({
          title:
            'Your password was accepted but the account would not load. Try again.',
      ...
      setFailure({
        title: errorMessage(error, OFFLINE_MESSAGE),
        body: WRONG_DETAILS_BODY,
      });
    }
  };
          ...
          <div className="-mt-2 flex flex-wrap items-center justify-between gap-4">
            <label className="flex cursor-pointer items-center gap-3 text-base">
              <Checkbox
                ...
                className="size-5 rounded-sm border-ui-hint/70 data-checked:border-ui-blue data-checked:bg-ui-blue"
              />
