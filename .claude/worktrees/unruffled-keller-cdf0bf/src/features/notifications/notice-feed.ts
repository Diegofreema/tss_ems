...
function feedItem(notice: Notice, now: Date): Notification | null {
  ...
  return {
    ...
    // The office writes the message in the rich-text editor, so it arrives as
    // HTML; a feed row wants the words alone, and the board's own detail view
    // is where the formatting shows. An editor emptied and saved still stores
    // `<p></p>`, which plainText reads as nothing at all.
